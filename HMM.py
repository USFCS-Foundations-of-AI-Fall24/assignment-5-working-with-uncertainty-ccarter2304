...
class HMM:
   ...
   #One cool thing we can do with an HMM is Monte Carlo simulation.
    ...
    def forward(self, sequence):
        ...
        #print(intial)
        seq = sequence.split(' ')
        seq_row = [""] * 2 + [s.strip() for s in seq]
        m.append(seq_row)
        for i in intial :
            if i == '#':
                i_row = [i] + [1.0] + [0] * len(seq)
            else :
                i_row = [i] + [0] * (len(seq) + 1)
            m.append(i_row)
        ...
        for k in range(len(intial) - 1) :
            if self.emissions.get(m[k + 2][0], {}).get(m[0][2]) is not None and self.transitions.get("#", {}).get(m[k + 2][0]) is not None :
        ...
        for j in range(3, len(seq) + 2) :
            ...
            for state in intial :
                ...
                for s2 in intial :
                    ...
                    if self.emissions.get(state, {}).get(m[0][j]) is not None and self.transitions.get(s2, {}).get(state) is not None:
        ...
        probable_state_val = m[2][len(seq) + 1]
        probable_state_indx = 2
        for i in range (3, len(intial)) :
            if m[i][len(seq) + 1] > probable_state_val :
                probable_state_val = m[i][len(seq) + 1]
...
if __name__ == '__main__':
    ...
    hm.load(basename)
    if func == '--generate' :
        print(hm.generate(int(param)))

    if func == '--forward' :
        r = open(param).readlines()
        for line in r :
            if line.strip() :
                hm.forward(line)
